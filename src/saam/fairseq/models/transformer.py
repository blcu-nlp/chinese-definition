...
class TransformerDecoderLayer(nn.Module):
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, no_encoder_attn=False):
        ...
        self.affine_residual = Linear(self.embed_dim * 2, 1)
        self.affine_x = Linear(self.embed_dim * 2, 1)
    ...
    def forward(self, x, encoder_out, encoder_padding_mask, incremental_state,
        ...
        if self.encoder_attn is not None:
            ...
            sememe = torch.cat([x, origin], dim=-1)
            sememe = self.affine_x(sememe)
            semantic = torch.cat([residual, origin], dim=-1)
            semantic = self.affine_residual(semantic)
            alpha = F.softmax(torch.cat([sememe, semantic], dim=-1), dim=-1)
            x = alpha[:, :, 0].unsqueeze(2) * x + alpha[:, :, 1].unsqueeze(2) * residual
            # x = residual + x
